...
function load_property_grid() {
    var options = { visit: 'loadyttables', ProjectID: ProjectID, PropertyID: PropertyID, sessionId: sessionId };
    console.log('options参数');
    console.log(options);
    $.ajax({
        type: 'POST',
        dataType: 'json',
...
function getmaxlevel(typedesc, titlevalue) {
    var returnvalue = 0;
    var jsonarry = [];
    var options = { visit: 'getmaxlevel', ID: ProjectID, sessionId: sessionId };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        async: false,
        // url: '../Handler/ProjectHandler.ashx',
        url: window.SERVERPATH + 'api/getmaxlevel',
        data: options,
        success: function (message) {
            // jsonarry = message;
            jsonarry = message.list;
...
function newsaveyt() {
    ...
    top.$.messager.confirm("提示", "是否提交?", function (r) {
        if (r) {
            ...
            var options = { visit: 'savesubproject', strjson: JSON.stringify(strarry), ProjectID: ProjectID, ProjectName: ProjectName, PropertyID: PropertyID, sessionId: sessionId };
            // ===== update =====
            ...
            $.ajax({
                ...
                dataType: 'html',
                // url: '../Handler/ProjectHandler.ashx',
                // url: 'URL:http://demo.saasyy.com/Handler/ProjectHandler.ashx',
                url: window.SERVERPATH + "api/createproject",
                data: options,
                success: function (message) {
                    ...
                    console.log("新增测试");
                    console.log(message);
                    console.log(message.status == true);
                    // console.log(message.status == "true");
                    // ===== update =====
                    // message = 0;
                    // ===== end =====
